  ...
  return (
    <Card className={`h-full flex flex-col mac-translucent-ruby ${className}`}>
      ...
      <CardContent className="space-y-4 overflow-y-auto flex-1 p-4 text-sm text-white/90">
        ...
        with incrementing counters and truncating the output to the number of
        rgb bytes in the masked region. this keystream is xor-ed with the
        ...
        them. the original video does not exist inside it as an extractable
        asset. it exists only as encrypted bytes plus per-frame visual payloads
        that are mathematically bound to a specific burn video and a separate
        key file.
        <br />
        <br />
        ...
        underlying stream to dump. mp4p does not store a hidden video waiting to
        be unlocked. it stores the conditions under which a video can be
        reconstructed, and outside of those conditions the format resolves only
        to encrypted media and synchronized noise.
        <br />
        <br />
        improvements
        <br />
