...
export function StreamPage({ onStatsChange }: StreamPageProps = {}) {
  ...
  const [sam3AssetPath, setSam3AssetPath] = useState<string | null>(null);
  ...
  const [sam3NoDetections, setSam3NoDetections] = useState(false);
  const [sam3BoxPromptEnabled, setSam3BoxPromptEnabled] = useState(false);
  const [sam3Box, setSam3Box] = useState<[number, number, number, number] | null>(
    null
  );
            ...
            <div className="flex relative gap-2 md:gap-4 w-full h-full max-w-[900px] flex-col md:flex-row">
              <div className="w-full md:w-64 h-full">
                <InputAndControlsPanel
                  ...
                  sam3NoDetections={sam3NoDetections}
                  sam3BoxPromptEnabled={sam3BoxPromptEnabled}
                  sam3Box={sam3Box}
